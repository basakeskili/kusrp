...
def get_args():
    ...
    parser.add_argument("--save", action='store_true', default=False, help="save param values to a file with the same basename as log")
...
class Reporter(object):
    ...
    def __init__(self, dset, feat):
        self.feat = feat
        self.tfunc = (rep.get_ts_bio if dset.tagging == 'bio' else rep.get_ts_io) if dset.level == 'char' else lambda x,y: y

        # self.tdecoder = decoder.ViterbiDecoder(dset.trn, feat) if dset.level == 'char' else decoder.MaxDecoder(dset.trn, feat)
        self.tdecoder = (decoder.ViterbiDecoder(dset.trn, feat) if dset.tagging == 'bio' else decoder.MaxDecoder(dset.trn, feat))\
                if dset.level == 'char' else decoder.WDecoder(dset.trn, feat)
            ...
            # for ddat, datname, dset in zip([self.devdat, self.tstdat],['dev','tst'], [self.dev, self.tst]):
                ...
                if f1 > dbests[datname][1]:
                    ...
                    if argsd['save'] and datname == 'dev': # save model to file
                        ...
                        np.savez('{}/{}'.format(MODEL_DIR, argsd['log']), argsd=argsd, rnn_param_values=rnn_param_values)
